...
public class SolLun {
	...
	public String SolToLun(int nowDate){
		try{
			if( nowDate < 10000000 ){
				return "";
			}
			String n = Integer.toString(nowDate);

			int SolYear		= Integer.parseInt( n.substring(0,4) );
			int SolMonth	= Integer.parseInt( n.substring(4,6) );
			int SolDay		= Integer.parseInt( n.substring(6,8) );

			return SolToLun(SolYear, SolMonth, SolDay);
		}catch(Exception e){
	...
	public String SolToLun(int SolYear, int SolMonth, int SolDay) {
		...
		if(kk[i].substring(12,13) != "0") temp = 13; 
		else temp = 12;
		...
		if((kk[LunYear - 1881].substring(12,13) != "0") && (Integer.parseInt(kk[LunYear - 1881].substring(m0)) > 2))
			isLeap = true;
		else
			isLeap = false;
	...
	public String LunToSol(int LunYear, int LunMonth, int LunDay, boolean isLeap) {
		...
		if(LunYear != 1881) {
			...
			for(i = 0; i <= yy; i++) {
				...
				if(kk[i].substring(12,13) == "0") acc_day = acc_day + 336;
		...
		// Leap Year
		if(isLeap) acc_day = acc_day + 28 + Integer.parseInt(kk[yy].substring(mm,mm+1));
